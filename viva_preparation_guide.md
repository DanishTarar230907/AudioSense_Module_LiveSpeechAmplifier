...
> *   **Local Sample Demonstrator:** To demonstrate this unzipped structure directly during our viva, we have generated a clean, compact local sample dataset folder inside our project repository at `ai_models/dataset_samples/`. This folder contains valid, unzipped `.wav` samples of clean speech formants and simulated ambient background noises (fan hum, white noise) at our exact target specification (16kHz, 16-bit, mono) so the examiners can inspect how our training data looks."
